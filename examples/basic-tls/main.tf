...
module "warpstream" {
  ...
  ecs_service_additional_iam_policies = [
    {
      name        = "tls-bucket",
      policy_json = data.aws_iam_policy_document.ec2_ecs_task_s3_tls.json
    }
  ]
  ecs_service_additional_environment_variables = [
    ...
    {
      name  = "WARPSTREAM_TLS_BLOB_URL"
      value = "s3://${aws_s3_bucket.tls_bucket.bucket}"
    },
    {
